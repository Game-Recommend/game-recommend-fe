...
진행 표시는 `src/lib/recommendation.ts`의 `PIPELINE_FLOW`를 고정으로 그립니다. 질문 분해 → 게임 검색 →
가격·하드웨어 → 리뷰 요약·미디어 → 최종 답변 생성의 다섯 칸을 선으로 잇고, 병렬로 도는 단계는 한 칸 안에서
두 갈래로 벌립니다. 지금 도는 칸만 연두로 밝히며, 백엔드가 함께 보내는 "조건 판정"은 결과 목록에 그대로
드러나므로 칸을 두지 않습니다.
